...
u32 rd(u32 x) 
{
    return ( (x & RD_MASK  ) >> _11_07_OFFSET );
}

u32 rs1(u32 x) 
{
    return ( (x & RS1_MASK ) >> _19_15_OFFSET );
}

u32 rs2(u32 x) 
{
    return ( (x & RS2_MASK ) >> _24_20_OFFSET );
}

i32 clr(u32 x) 
{
    *_I32(&VM.M[x]) = 0; return 0;
}

i32 offset(u32 x) 
{
    return (I32($rs1)+decode_imm(x));
}
